...
<head>
    ...
    <title>@yield('title', $siteSettingsService->get('branding.site_name'))</title>
    ...
    @stack('page-styles')
    @php
        $usesAdminShell = auth()->check() && (auth()->user()->can('isAdminOrStaff') || auth()->user()->can('isAdminDeveloper'));
    @endphp
</head>
<body class="@yield('body_class') {{ $usesAdminShell ? 'admin-shell-body' : '' }}">
    @if($usesAdminShell)
